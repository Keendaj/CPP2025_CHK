...
void startCalculations() {
    ...
    while (true) {
        ...
        if (input == "exit") break;
        if (input == "help") {
            std::cout << "Введите математическое выражение в инфиксной нотации\n"
                    << "Доступны: + - * / ( )\n"
                    << "Унарный плюс не поддерживается.\n"
                    << "Унарный минус поддерживается только при использовании его в начале выражения либо после открывающей скобки.\n"
                    << "Также вы можете добавить свои функции через плагины в /plugins, подробнее в ReadMe\n"
                    << "Пример: 5 + 2 * 3 / 1"
                    << std::endl;
            continue;
        }
